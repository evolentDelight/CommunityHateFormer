...
from wrapper import MyGraphPropPredDataset, MyPygPCQM4MDataset, MyZINCDataset, RedditDataset
...
def get_dataset(dataset_name='abaaba', subreddit=None):
    global dataset
    ...
    if dataset_name == 'ogbg-molpcba':
        ...
        dataset = {
            ...
            # 'loss_fn': torch.nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([5, 5, 5, 5]).to('cuda:0')), 
            'metric': 'mse',
            'metric_mode': 'min',
            'evaluator': F.l1_loss, # AP
            'dataset': RedditDataset('/home/l2hebert/dev/processed-graphs-bert/', subreddit=subreddit),
            'max_node': 300,
        }
    else:
...
class GraphDataModule(LightningDataModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.dataset = get_dataset(self.dataset_name, subreddit)
    ...
    def change_subreddit(self, subreddit):
        self.dataset = {
            'num_class': 1,
            'loss_fn': torch.nn.MSELoss(),
            # 'loss_fn': torch.nn.BCEWithLogitsLoss(pos_weight=torch.Tensor([5, 5, 5, 5]).to('cuda:0')), 
            'metric': 'mse',
            'metric_mode': 'min',
            'evaluator': F.mse_loss, # AP
            'dataset': RedditDataset('/home/l2hebert/dev/processed-graphs/', subreddit=subreddit),
            'max_node': 300,
        }
    ...
    def setup(self, stage: str = None):
        if self.dataset_name == 'ZINC':
            ...
            self.dataset_train = self.dataset_test = self.dataset_val = self.dataset['dataset']
        else:
    ...
    def train_dataloader(self):
        loader = DataLoader(
            ...
            pin_memory=False,
            collate_fn=partial(collator, mode='train', max_node=get_dataset(self.dataset_name)[
    ...
    def val_dataloader(self):
        loader = DataLoader(
            ...
            num_workers=self.num_workers,
            pin_memory=False,
            collate_fn=partial(collator, mode='val', max_node=get_dataset(self.dataset_name)[
                               'max_node'], multi_hop_max_dist=self.multi_hop_max_dist, spatial_pos_max=self.spatial_pos_max),
        )
    ...
    def test_dataloader(self):
        loader = DataLoader(
            ...
            num_workers=self.num_workers,
            pin_memory=False,
            collate_fn=partial(collator, mode='test', max_node=get_dataset(self.dataset_name)[
                               'max_node'], multi_hop_max_dist=self.multi_hop_max_dist, spatial_pos_max=self.spatial_pos_max),
        )
