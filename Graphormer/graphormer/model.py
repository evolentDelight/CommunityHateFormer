...
class Graphormer(pl.LightningModule):
    def __init__(
        ...
        if dataset_name == 'ZINC':
            ...
            self.spatial_pos_encoder = nn.Embedding(512, num_heads)
            self.in_degree_encoder = nn.Embedding(
                512, hidden_dim)
            self.out_degree_encoder = nn.Embedding(
                512, hidden_dim)
    ...
    def forward(self, batched_data, perturb=None):
       ...
       # graph_attn_bias[:, :, :, 0] = graph_attn_bias[:, :, :, 0] + t
        ...
        # node feauture + graph token # EDIT Removed the cat
        ...
        elif self.dataset_name == 'REDDIT': # predictions for everything 
            output = self.downstream_out_proj(output)
            #output = torch.clamp(output, 0, 4)
        else:
    ...
    def training_step(self, batched_data, batch_idx):
        if self.dataset_name == 'ogbg-molpcba':
            ...
            y_hat = self(batched_data).squeeze(-1)
            y_gt = batched_data.y.flatten(-1)
            mask = batched_data.mask.flatten(-1)
            
            #print(mask.shape)
            #y_hat, y_gt = y_hat.flatten(end_dim=-1), y_gt.view(-1).unsqueeze(1)
            
            y_hat, y_gt = y_hat[mask], y_gt[mask]
            #print(y_hat.shape, y_gt.shape)

            #y_hat, y_gt = y_hat[batched_data.mask], y_gt[batched_data.mask]
    ...
    def validation_step(self, batched_data, batch_idx):
        if self.dataset_name in ['PCQM4M-LSC', 'ZINC']:
            ...
            y_pred = self(batched_data).squeeze(-1)
            y_pred = y_pred[mask]
            

            val_loss = self.evaluator(y_pred, y_true)

            # for i in range(4):
            #     self.log('val_'+str(i)+'_bce_loss', val_loss[i])
    ...
    def validation_epoch_end(self, outputs):
        ...
        # losses0 = torch.cat([loss['val_loss_0'].flatten() for loss in outputs]).mean()
        # losses1 = torch.cat([loss['val_loss_1'].flatten() for loss in outputs]).mean()
        # losses2 = torch.cat([loss['val_loss_2'].flatten() for loss in outputs]).mean()
        # losses3 = torch.cat([loss['val_loss_3'].flatten() for loss in outputs]).mean()
        try:
            self.log('valid_mse', losses)
            # self.log('valid_cross_entropy0', losses0)
            # self.log('valid_cross_entropy1', losses1)
            # self.log('valid_cross_entropy2', losses2)
            # self.log('valid_cross_entropy3', losses3)
        except:
            pass
        # y_pred = torch.cat([i['y_pred'].flatten() for i in outputs])
        # y_true = torch.cat([i['y_true'].flatten() for i in outputs])
        # if self.dataset_name == 'ogbg-molpcba':
    ...
    def test_step(self, batched_data, batch_idx):
        if self.dataset_name in ['PCQM4M-LSC', 'ZINC']:
            ...
            test_loss = self.evaluator(y_pred, y_true)

            # for i in range(4):
            #     self.log('test_'+str(i)+'_bce_loss', test_loss[i])
    ...
    def test_epoch_end(self, outputs):
        ...
        self.log('test_mse', losses)
        # losses0 = torch.cat([loss['test_loss_0'].flatten() for loss in outputs]).mean()
        # losses1 = torch.cat([loss['test_loss_1'].flatten() for loss in outputs]).mean()
        # losses2 = torch.cat([loss['test_loss_2'].flatten() for loss in outputs]).mean()
        # losses3 = torch.cat([loss['test_loss_3'].flatten() for loss in outputs]).mean()
        # self.log('test_cross_entropy0', losses0)
        # self.log('test_cross_entropy1', losses1)
        # self.log('test_cross_entropy2', losses2)
        # self.log('test_cross_entropy3', losses3)
        # y_pred = torch.cat([i['y_pred'].flatten() for i in outputs])
        # y_true = torch.cat([i['y_true'].flatten() for i in outputs])
        # if self.dataset_name == 'PCQM4M-LSC':
