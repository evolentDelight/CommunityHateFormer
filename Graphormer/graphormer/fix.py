...
for path in tqdm.tqdm(data['idx_path'].unique(), total=(len(data['idx_path'].unique()))):
    try:
        x = torch.load('/home/l2hebert/dev/processed-graphs-bert/processed/' + path)
        ...
        x = preprocess_item(x)
        x = torch.save(x ,'/home/l2hebert/dev/processed-graphs-bert/processed/' + path)
    except:
        skipped += [path]
        print('skipped2', path)
